...
| [ROW_NAME_FROM_SYNONYM.md](./ROW_NAME_FROM_SYNONYM.md) | The opt-in feature that names a stoichiometry row from the molecule's shortest synonym and remembers names typed by hand: why shortest, the three scopes, the baseline rule, and the two DOM traps the Name field sets. | You're testing, tuning or extending that feature. |
| [ELN_ID_PRODUCT_SUFFIX.md](./ELN_ID_PRODUCT_SUFFIX.md) | The mark that says which stoichiometry table a registered product came from: the two settings and their six combinations, why parallel reactions ignore them, and the one function that decides it. | You're changing how a product ID is composed, or a suffix came out wrong. |
...
| [REGISTRATION_FORM_FIELD_FILTER.md](./REGISTRATION_FORM_FIELD_FILTER.md) | Research (not yet built) for filtering the Search/Inventory field pickers by registration form: where CDD keeps the form→fields mapping, why name matching is the only join and why it's safe, measured payoff, and the agreed design. | You're implementing that filter, or you need CDD's `registration_form_definitions` for something else. |
