...
// added at the end of its batch table, nothing (it has them), or never (it
// has no layout of its own, or one this extension does not recognise).
//
...
import { PLAN_ADD, planForm, putBody, resolveChosen, sameDocument, verifySaved, withRows } from "./row-model.js";
...
export function buildBar() {
    ...
    openButton.addEventListener("click", async () => {
        ...
        openCard(forms, map.defs.batch.filter((def) => def && !def.disabled), remembered);
    });

    function openCard(forms, defs, remembered) {
        card.textContent = "";
        card.hidden = false;
        ...
        function judge() {
            ...
            for (const row of formRows) {
                const plan = cells.length ? planForm(row.form, cells) : { status: null, note: "" };
                const can = plan.status === PLAN_ADD;
                row.plan = plan;
        ...
        function count() {
            ...
            action.textContent = `Add to ${plural(n, "form")}`;
        ...
        action.addEventListener("click", async () => {
            ...
            try {
                ...
                for (const row of rows) {
                    ...
                    const expected = withRows(fresh, cells);
                    ...
                    row.why.textContent = "added";
                    made += 1;
                }
                status.textContent = `Added to ${plural(made, "form")}. Reloading…`;
                setTimeout(() => location.reload(), 1200);
            } catch (error) {
                cancel.disabled = false;
                status.textContent = `Added to ${made} of ${rows.length}. Stopped at "${current?.form.name ?? ""}" — ${error.message}.`;
            }
        });
    }
