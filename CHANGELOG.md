...
### Added
- **Duplicate sample** (`features/sample-duplicate.js`). A fourth icon next to
  CDD's Deplete / Edit / New event icons on every sample of a molecule's
  *Samples* tab. It opens CDD's own *Create a New Sample* dialog filled in
  like that sample: Batch, Units, Initial Amount (= the parent's **current**
  amount) and every sample field the dialog shows, matched by field name.
  Sample ID (autogenerated), Barcode (unique), the location and the event
  fields (Comment, Freezing cycle…) stay empty — the user picks a location
  and saves. Nothing is saved by the extension; the data are read fresh from
  `inventory_samples.json` at the click. Fields that could not be filled are
  listed in a note at the top of the dialog. The multi-position Batch Create
  bar works in the dialog as usual, so several picked positions give several
  copies.
...
- **Plate QC above a run's Data Summary** (`/vaults/<v>/runs/<r>`). Per
  ...
  window SW = |Δmean|/(SD(pos) + SD(neg)),
  Z′ = 1 − 3·(SD(pos) + SD(neg))/|Δmean|, robust Z′ (median for mean,
  1.4826·MAD for SD) and plate drift of each control — the plate's mean vs
  the run's average plate, in %; a footer row gives the run's mean ± SD of
  each metric (not of drift, which averages to 0). Z′ and robust Z′ below
  0.5 and drift beyond ±20 % are shown in red. Every column header carries
  a tooltip with the metric's meaning and formula, every value one with the
  formula filled in with that plate's numbers. A readout picker
...
- The panel folds to its title line (▸ / ▾) and every run starts folded —
  whoever wants the table opens it; nothing is remembered. Folded, it
  fetches nothing — the heat maps
